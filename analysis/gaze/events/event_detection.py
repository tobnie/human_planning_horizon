...
def fixation_detection(x, y, time, missing=-32768, maxdist=25, mindur=50):
...
def get_fixations_from_samples(samples):
    if isinstance(samples, list) and not samples:
        return [], []
    if isinstance(samples, np.ndarray) and samples.shape[0] == 0:
        return [], []
    time, x, y, _ = list(zip(*samples))
    return fixation_detection(np.array(x), np.array(y), np.array(time))


def saccade_detection(x, y, time, missing=-32768, minlen=5, maxvel=40, maxacc=340):
...
def try_blink_detection(x, y, time):
    try:
        results = blink_detection(np.array(x), np.array(y), np.array(time))
        return results
